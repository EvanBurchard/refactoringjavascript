const classifier = {
  ...
  train: function(chords, label){
    ...
    if(Array.from(this.labelCounts.keys()).includes(label)){
      this.labelCounts.set(
        label, this.labelCounts.get(label) + 1);
    } else {
  ...
  setLabelProbabilities: function(){
    this.labelCounts.forEach(function(_count, label){
      this.labelProbabilities.set(
        label, this.labelCounts.get(label) / this.songList.songs.length);
    }, this)
  },
...
describe('the file', function() {
  ...
  classifier.songList.addSong('paperBag', ['bm7', 'e', 'c', 'g',
              'b7', 'f', 'em', 'a',
              'cmaj7', 'em7', 'a7', 'f7',
              'b'], 2);
